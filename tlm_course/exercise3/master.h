...
private:
    ...
    // state variables
    unsigned int resp_pending_;
    // keep track of the transaction that needs an END_REQ
    tlm_generic_payload *trans_pending_req_; 

    sc_core::sc_event req_event_;// event to be notified if END_REQ comes in
    sc_core::sc_event resp_event_;//event to be notified if all responses are in
